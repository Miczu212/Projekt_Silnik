...
public:
	...
	static SoundHandler& Get(); //funkcja kt鏎a upewnia nas ze system zostanie zainicjalizowany tylko raz
		...
		{
			{
				...
				catch (std::exception e)
				{
					throw SoundHandler::FileError(
						std::string("B章d Podczas Pr鏏y Otworzenia Pliku: ") + e.what());
				}
				...
				if (fileSize <= 44)
				{
					throw SoundHandler::FileError("Plik jest za ma造");
				}
			...
			// compare format with sound system format
			{
				...
				{
					throw SoundHandler::FileError("Z造 Format Pliku (nChannels)");
				}
				else if (format.wBitsPerSample != sysFormat.wBitsPerSample)
				{
					throw SoundHandler::FileError("Z造 Format Pliku (wBitsPerSample)");
				}
				else if (format.nSamplesPerSec != sysFormat.nSamplesPerSec)
				{
					throw SoundHandler::FileError("Z造 Format Pliku (nSamplesPerSec)");
				}
				else if (format.wFormatTag != sysFormat.wFormatTag)
				{
					throw SoundHandler::FileError("Z造 Format Pliku (wFormatTag)");
				}
				else if (format.nBlockAlign != sysFormat.nBlockAlign)
				{
					throw SoundHandler::FileError("Z造 Format Pliku (nBlockAlign)");
				}
				else if (format.nAvgBytesPerSec != sysFormat.nAvgBytesPerSec)
				{
					throw SoundHandler::FileError("Z造 Format Pliku (nAvgBytesPerSec)");
				}
			}
